...
RSpec.shared_examples 'user not authenticated' do
  ...
  before do
    ...
    allow(controller).to receive(:dbhero_current_user).and_return(nil)
  end

  it 'raises forbidden error' do
    expect do
      send(method_name, *options)
    end.to raise_error(ActionController::RoutingError, 'Forbidden')
  end
end
...
RSpec.shared_examples 'user authenticated' do
  ...
  before do
    ...
    allow(controller).to receive(:dbhero_current_user).and_return(current_user)
    allow(Dbhero).to receive(:authenticate).and_return(true)
...
RSpec.shared_examples 'user authenticated match for custom role' do
  ...
  before do
    ...
    allow(controller).to receive(:dbhero_current_user).and_return(current_user)
    allow(Dbhero).to receive(:authenticate).and_return(true)
    allow(Dbhero).to receive(:custom_user_auth_condition).and_return(lambda { |u|
                                                                       u.admin?
                                                                     })
...
RSpec.describe Dbhero::DataclipsController, type: :controller do
  ...
  describe 'GET index' do
    describe 'with enabled auth' do
      it_behaves_like 'user not authenticated' do
        let(:options) { :index }
      end

      it_behaves_like 'user authenticated' do
        let(:options) { :index }
      end

      it_behaves_like 'user authenticated match for custom role' do
        let(:options) { :index }
      end

      it_behaves_like "user authenticated don't match for custom role" do
        let(:options) { :index }
      end
    end

    describe 'with disabled auth' do
      it_behaves_like 'disabled auth' do
        let(:options) { :index }
      end
    end
  end
  ...
  describe 'POST create' do
    describe 'with enabled auth' do
      it_behaves_like 'user not authenticated' do
        ...
        let(:options) do
          [:create,
           { params: { dataclip: { description: 'foo bar',
                                   raw_query: "select 'foo' as bar" } } }]
        end
      end
      ...
      it_behaves_like 'user authenticated' do
        ...
        it {
          expect(Dbhero::Dataclip.find_by(description: 'foo bar')).not_to be_nil
        }
      end
      ...
      it_behaves_like 'user authenticated match for custom role' do
        ...
        it 'find dataclip' do
          ...
          expect(clip.user).to eq(controller.dbhero_current_user.email)
    ...
    describe 'with disabled auth' do
      it_behaves_like 'disabled auth' do
        ...
        let(:options) do
          [:create,
           { params: { dataclip: { description: 'foo bar disabled',
                                   raw_query: "select 'foo' as bar" } } }]
        end
        let(:expect_method) { :be_redirect }
  ...
  describe 'PUT update' do
    describe 'with enabled auth' do
      ...
      it_behaves_like 'user authenticated' do
        ...
        let(:dataclip) do
          create(:dataclip,
                 { description: 'foo bar', raw_query: "select 'foo' as bar" })
        end
        let(:options) do
          [:update,
           { params: { id: dataclip.token,
                       dataclip: { description: 'updated' } } }]
        end
        let(:expect_method) { :be_redirect }
